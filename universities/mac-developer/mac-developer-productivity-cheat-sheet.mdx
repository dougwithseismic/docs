...
<Info>
  **Common shortcuts**: `Cmd+Space` (Spotlight/Raycast), `Cmd+Tab` (app
  switching), `Cmd+Shift+.` (show hidden files), and `Space` (Quick Look).
</Info>
...
<Info>
  Terminal shortcuts like `Ctrl+A` (beginning) and `Ctrl+E` (end) keep your
  hands on the home row, reducing hand movement.
</Info>
...
<Info>
  **Useful VS Code shortcuts**: `Cmd+P` for Quick Open (faster than file tree
  navigation) and `Cmd+Shift+P` for the command palette.
</Info>
...
<Info>
  **Git shortcuts**: These aliases reduce typing. For example, `gacp` (add,
  commit, push) combines three commands into one.
</Info>
...
<Info>
  **Common DevTools shortcuts**: `Cmd+Option+I` to open DevTools, `Cmd+K` to
  clear console, and F8-F11 for debugging.
</Info>
...
<Info>
  Learn terminal commands as needed. Start with `ls`, `cd`, `grep`, and `kill`.
  Add others when you use them frequently.
</Info>
...
<Accordion title="What if a shortcut doesn't work in my app?">
  Apps can override system shortcuts. Check the app's preferences for custom
  keybindings. You can often reassign shortcuts in System Settings → Keyboard →
...
<Accordion title="Should I remap my keyboard keys?">
  Consider remapping Caps Lock to Escape or Control if you don't use HyperKey.
...
<Accordion title="How do I find an app's shortcuts?">
  Most apps list shortcuts next to menu items. Press and hold `Cmd` in many apps
...
<Accordion title="Which shortcuts save the most time?">
  Based on usage data: `Cmd+Tab` (app switching), `Cmd+Space` (launcher),
  `Cmd+C/V` (copy/paste), `Cmd+Z` (undo), and `Cmd+W` (close). These five
  account for 50%+ of shortcut usage.
</Accordion>

{" "}

<Accordion title="Can I create my own shortcuts?">
  Yes! System Settings → Keyboard → Shortcuts lets you add app-specific
...
_Want to set up your Mac for maximum productivity? Check our complete [Mac Developer Essentials](./initial-setup) for installations, configurations, and automation scripts._
