...
{`{
  ...
  "url": "https://docs.withseismic.com/case-studies/getbenson-discount-extension-blocker",
  "author": {
    "@type": "Organization",
    "name": "WithSeismic"
  },
...
As Honey users ourselves, we witnessed firsthand how extensions earned affiliate commissions without providing real value. The codes often didn't work, yet brands were still paying for these failed conversions.
...
Before building, we researched existing solutions. While a couple existed, none offered comprehensive protection. Our vision crystallized around three core questions: Could we reliably detect these extensions? Could we prevent them from interfering? Could we help brands reclaim their lost margin?

With clients from our affiliate network already expressing frustration about this problem, the decision to build getBenson was obvious.
...
We experimented with countless detection strategies, each iteration teaching us more about the delicate balance between functionality and security. The key insight: maintaining strict separation between the extension world and our protected checkout environment was non-negotiable.

### The Breakthrough

Our solution came from an unexpected approach:

The breakthrough came when we approached the problem like reverse engineering an application. By detecting specific files that extensions exposed through the browser's API, we could create unique fingerprints for each extension - definitively identifying whether Honey, Retail Me Not, Rakuten, or others were active on the page.
...
Theory11, a premium playing card brand, became our first client. They had already experienced Honey's devastating impact on their margins and tried other solutions without success. For them, getBenson was an immediate necessity.
...
These brands had clear urgency: "Block these extensions now - we're hemorrhaging margin." They understood the problem and wanted immediate protection.

**The Data-Driven Analysts**
More sophisticated buyers took a measured approach: "Before blocking anything, we need to understand the true impact." This feedback pushed us to evolve getBenson beyond simple blocking into a comprehensive analytics platform that could quantify exactly how much these extensions were costing.
...
If we were building getBenson today, we'd take a fundamentally different approach. Rather than positioning ourselves as adversaries to discount extensions, we'd focus on objective impact analysis. The question wouldn't be "How do we block them?" but "What's the actual ROI of these partnerships?"

We'd even explore partnering with the extensions themselves, positioning getBenson as a neutral analytics platform that helps brands make informed decisions rather than pushing a narrative about good versus bad.

### The Partnership Paradox

Our original approach created unnecessary enemies:

This collaborative approach would align with affiliate networks' interests rather than opposing them. Our original adversarial stance put us at odds with networks who benefited from extension commissions - why would they support something that reduced their revenue?

### The Maturity of Perspective

Experience has taught us that adversarial strategies have limited reach. True success comes from creating value for all stakeholders, providing transparent data that empowers informed decisions rather than forcing a particular narrative.
