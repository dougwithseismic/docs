...
{`{
  ...
  "url": "https://docs.withseismic.com/case-studies/lead-qualifier",
  "author": {
    "@type": "Person",
    "name": "Doug Silkstone"
  },
...
const detectEngagementSignals = (profile) => {
  ...
  const avgTimePerPage = profile.behavior.totalTimeSpent /
                         profile.behavior.totalPageViews;
  ...
  // Content depth
  const deepPages = Object.values(profile.behavior.pages)
    .filter(page => page.maxScrollDepth >= 75);
